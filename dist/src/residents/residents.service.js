...
let ResidentsService = ResidentsService_1 = class ResidentsService {
    ...
    async create(createResidentDto) {
        ...
        if (!isAutoGenerated) {
            const existingResident = await this.residentsRepository.findByResidentCode(providedCode);
            if (existingResident) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            try {
                ...
                if (isAutoGenerated && isUniqueViolation && attempt < maxAttempts) {
                    this.logger.warn(`Resident code collision on attempt ${attempt}, regenerating...`);
                    continue;
                }
        ...
        throw new common_1.ConflictException('Unable to generate a unique resident code, please try again');
    }
