import { EmployeeSalaryHeadersRepository } from './employee-salary-headers.repository';
import { CreateEmployeeSalaryHeaderDto } from './dto/create-employee-salary-header.dto';
import { UpdateEmployeeSalaryHeaderDto } from './dto/update-employee-salary-header.dto';
import { QueryEmployeeSalaryHeadersDto } from './dto/query-employee-salary-headers.dto';
import { CreateSimplePayrollDto } from './dto/create-simple-payroll.dto';
import { PrismaService } from '../prisma/prisma.service';
import { CashTransactionsService } from '../cash-transactions/cash-transactions.service';
export declare class EmployeeSalaryHeadersService {
    private readonly repository;
    private readonly prisma;
    private readonly cashTransactionsService;
    constructor(repository: EmployeeSalaryHeadersRepository, prisma: PrismaService, cashTransactionsService: CashTransactionsService);
    create(createEmployeeSalaryHeaderDto: CreateEmployeeSalaryHeaderDto): Promise<{
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    findAll(queryDto: QueryEmployeeSalaryHeadersDto): Promise<{
        headers: import(".prisma/client").EmployeeSalaryHeader[];
        total: number;
    }>;
    findById(id: string): Promise<any>;
    update(id: string, updateEmployeeSalaryHeaderDto: UpdateEmployeeSalaryHeaderDto): Promise<{
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    remove(id: string): Promise<{
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    calculateSalary(id: string): Promise<any>;
    private calculateComponentAmount;
    approveSalary(id: string, approverId: string): Promise<{
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    createSimplePayroll(dto: CreateSimplePayrollDto, userId: string): Promise<{
        cashTransaction: {
            id: string;
            description: string | null;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            status: string;
            approvedBy: string | null;
            approvedAt: Date | null;
            createdBy: string;
            transactionNumber: string;
            transactionDate: Date;
            transactionType: string;
            amount: import("@prisma/client-runtime-utils").Decimal;
            categoryId: string | null;
            referenceType: string | null;
            referenceId: string | null;
            cashAccountId: string | null;
            isInternalTransfer: boolean;
            transferGroupId: string | null;
        };
        employee: {
            position: {
                id: string;
                description: string | null;
                isActive: boolean;
                createdAt: Date;
                updatedAt: Date;
                deletedAt: Date | null;
                positionCode: string;
                positionName: string;
                department: string;
                salaryGrade: string | null;
                level: number | null;
            };
        } & {
            id: string;
            isActive: boolean;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            email: string | null;
            firstName: string;
            lastName: string;
            phoneNumber: string | null;
            roleId: string | null;
            address: string | null;
            userId: string | null;
            alternatePhone: string | null;
            identityNumber: string | null;
            dateOfBirth: Date | null;
            gender: string | null;
            maritalStatus: string | null;
            emergencyContact: string | null;
            emergencyPhone: string | null;
            employeeCode: string;
            city: string | null;
            province: string | null;
            postalCode: string | null;
            positionId: string;
            hireDate: Date;
            probationEndDate: Date | null;
            employmentStatus: string;
            basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
            bankName: string | null;
            bankAccountNumber: string | null;
            bankAccountName: string | null;
            taxId: string | null;
            photo: string | null;
            notes: string | null;
        };
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    markAsPaid(id: string, paymentDate?: Date, paidBy?: string): Promise<{
        employee: {
            position: {
                id: string;
                description: string | null;
                isActive: boolean;
                createdAt: Date;
                updatedAt: Date;
                deletedAt: Date | null;
                positionCode: string;
                positionName: string;
                department: string;
                salaryGrade: string | null;
                level: number | null;
            };
        } & {
            id: string;
            isActive: boolean;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            email: string | null;
            firstName: string;
            lastName: string;
            phoneNumber: string | null;
            roleId: string | null;
            address: string | null;
            userId: string | null;
            alternatePhone: string | null;
            identityNumber: string | null;
            dateOfBirth: Date | null;
            gender: string | null;
            maritalStatus: string | null;
            emergencyContact: string | null;
            emergencyPhone: string | null;
            employeeCode: string;
            city: string | null;
            province: string | null;
            postalCode: string | null;
            positionId: string;
            hireDate: Date;
            probationEndDate: Date | null;
            employmentStatus: string;
            basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
            bankName: string | null;
            bankAccountNumber: string | null;
            bankAccountName: string | null;
            taxId: string | null;
            photo: string | null;
            notes: string | null;
        };
        details: ({
            component: {
                id: string;
                description: string | null;
                isActive: boolean;
                createdAt: Date;
                updatedAt: Date;
                deletedAt: Date | null;
                isTaxable: boolean;
                componentCode: string;
                componentName: string;
                componentType: string;
                calculationType: string;
                isFixed: boolean;
                fixedAmount: import("@prisma/client-runtime-utils").Decimal | null;
                percentage: import("@prisma/client-runtime-utils").Decimal | null;
                calculationBase: string | null;
                calculationOrder: number;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            notes: string | null;
            amount: import("@prisma/client-runtime-utils").Decimal;
            quantity: number;
            salaryHeaderId: string;
            componentId: string;
            manualOverride: boolean;
        })[];
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
    cancelPayroll(id: string, cancelledBy: string, reason?: string): Promise<{
        cancelledCashTransactionNumber: string | null;
        employee: {
            position: {
                id: string;
                description: string | null;
                isActive: boolean;
                createdAt: Date;
                updatedAt: Date;
                deletedAt: Date | null;
                positionCode: string;
                positionName: string;
                department: string;
                salaryGrade: string | null;
                level: number | null;
            };
        } & {
            id: string;
            isActive: boolean;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            email: string | null;
            firstName: string;
            lastName: string;
            phoneNumber: string | null;
            roleId: string | null;
            address: string | null;
            userId: string | null;
            alternatePhone: string | null;
            identityNumber: string | null;
            dateOfBirth: Date | null;
            gender: string | null;
            maritalStatus: string | null;
            emergencyContact: string | null;
            emergencyPhone: string | null;
            employeeCode: string;
            city: string | null;
            province: string | null;
            postalCode: string | null;
            positionId: string;
            hireDate: Date;
            probationEndDate: Date | null;
            employmentStatus: string;
            basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
            bankName: string | null;
            bankAccountNumber: string | null;
            bankAccountName: string | null;
            taxId: string | null;
            photo: string | null;
            notes: string | null;
        };
        details: ({
            component: {
                id: string;
                description: string | null;
                isActive: boolean;
                createdAt: Date;
                updatedAt: Date;
                deletedAt: Date | null;
                isTaxable: boolean;
                componentCode: string;
                componentName: string;
                componentType: string;
                calculationType: string;
                isFixed: boolean;
                fixedAmount: import("@prisma/client-runtime-utils").Decimal | null;
                percentage: import("@prisma/client-runtime-utils").Decimal | null;
                calculationBase: string | null;
                calculationOrder: number;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            deletedAt: Date | null;
            notes: string | null;
            amount: import("@prisma/client-runtime-utils").Decimal;
            quantity: number;
            salaryHeaderId: string;
            componentId: string;
            manualOverride: boolean;
        })[];
        id: string;
        createdAt: Date;
        updatedAt: Date;
        deletedAt: Date | null;
        basicSalary: import("@prisma/client-runtime-utils").Decimal | null;
        notes: string | null;
        status: string;
        createdBy: string;
        payrollNumber: string;
        employeeId: string;
        payPeriod: string;
        paymentDate: Date | null;
        totalAllowances: import("@prisma/client-runtime-utils").Decimal | null;
        totalDeductions: import("@prisma/client-runtime-utils").Decimal | null;
        netSalary: import("@prisma/client-runtime-utils").Decimal | null;
        workDays: number | null;
        daysWorked: number | null;
        overtimeHours: import("@prisma/client-runtime-utils").Decimal | null;
        leaveDays: number | null;
        locked: boolean;
    }>;
}
